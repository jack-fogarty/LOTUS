%% LOTUS: Movmedian Filter Data
%  Copyright (C) (2024) Jack Fogarty

function app = ep_movmedian(app,i)
    ...
    for sig = 1:length(signals)
        ...
        for t = 1:length(eval(sigdat))
            ...
            if ~strcmp(signals{sig,1},'ACC')
                ...
                % Apply median filter
                dat = medfilt1(eval([sigdat '{' num2str(t) ',1}.data']),app.fil{i,1}.cfg.winpts,'omitnan');
                ...
                % Apply median filter
                datx = medfilt1(eval([sigdat '{' num2str(t) ',1}.data_x']),app.fil{i,1}.cfg.winpts,'omitnan');
                daty = medfilt1(eval([sigdat '{' num2str(t) ',1}.data_y']),app.fil{i,1}.cfg.winpts,'omitnan');
                datz = medfilt1(eval([sigdat '{' num2str(t) ',1}.data_z']),app.fil{i,1}.cfg.winpts,'omitnan');
